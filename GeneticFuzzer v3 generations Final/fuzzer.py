...
def chose_input_probabilities(input_samples):
    ...
    #amount of blocks in coverage + uniqness
...
def run_cov(exename,input_name,iteration,sessionId):
    ...
    for cov in glob("./cov/*.sancov"):
        ...
        if not c.issubset(main_coverage):
            ...
            conv_id = uuid.uuid4()
            shutil.copyfile("test.sample","./inputs/input.{}.session:{},unique:{}.sample".format(iteration,sessionId,conv_id))

            children.append(load_file(input_name))
            #Add new coverage to coverages
            coverage_file  = open("./cov/coverage.{}.session:{},unique:{}.txt".format(iteration,sessionId,conv_id),"w")
    ...
    if ("ERROR" in str(error)) or ("WARNING" in str(error)):
        result = str(error)
        #print(str(error))
	#print("ERRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRROR")
        #print("Conv_id:",conv_id)
        return result,conv_id#.split("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk")[1]
    return None,None
...
for x in inputFiles:
    ...
    if output != None:
        print("CRASH")
        save_file("./crash/crash.{}.session:{}.unique:{}.txt".format(x,sessionId,"Original"), load_file(x))
        save_file("./crash/crash.sample.{}.session:{}.unique:{}.txt".format(x,sessionId,"Original"), output)
print("Coverages: " , parents_cov)
print(main_coverage)
...
for iteration in range(safetybarrier):
    ...
    output,idf = run_cov("./a.out","test.sample",i,sessionId)

    if output != None:
        #run with gdb
        print("CRASH")
        #print("IDF: ",idf)
        save_file("./crash/crash.{}.session:{}.unique:{}.txt".format(i,sessionId,idf), mutated_sample)
        save_file("./crash/crash.sample.{}.session:{}.unique:{}.txt".format(i,sessionId,idf), output)
...
if fullFuzzing:
    while True:
        ...
        if output != None:
            #run with gdb
            print("CRASH")
            save_file("./crash/crash.{}.session:{}.unique:{}.txt".format(i,idf,sessionId), mutated_sample)
            save_file("./crash/crash.sample.{}.session:{}.unique:{}.txt".format(i,idf,sessionId), output)
...
#solve the problem if unique identifiers for files 